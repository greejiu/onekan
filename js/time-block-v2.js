...
export function findFirstAvailableTimeBlockSlot(templates, assignments, occurrences, preferredBlockId = "", excludeToken = "") {
  ...
  for (const block of orderedBlocks) {
    ...
    const legacyAssignments = Object.entries(assignments || {})
      .filter(([token, value]) => token !== excludeToken && legacyTokens.has(String(token)) && String(value?.blockId || "") === String(block.id))
      .sort((a, b) => String(a[1]?.afterAnchor || TIME_BLOCK_START_ANCHOR).localeCompare(String(b[1]?.afterAnchor || TIME_BLOCK_START_ANCHOR)) || (Number(a[1]?.order) || 1) - (Number(b[1]?.order) || 1) || a[0].localeCompare(b[0]));
    for (const _assignment of legacyAssignments) {
      const reserved = candidates.find((minute) => !overlaps(minute, occupied));
