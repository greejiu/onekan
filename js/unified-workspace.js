...
let taskPageMode="list",taskListTab="all",taskCalendarView="month",taskCalendarLayout="board",taskCalendarCursor=fromKey(todayKey());
...
function renderScheduleSubnav(){const nav=$("#calendarViewSeg");if(!nav)return;if(schedulePageMode==="list"){nav.innerHTML="";return}const layout=calendarView==="month"?"board":scheduleCalendarLayout;nav.innerHTML=`<div class="uw-task-calendar-tabs"><div class="seg">${[["month","월"],["week","주"],["day","일"]].map(([id,label])=>`<button class="${calendarView===id?"active":""}" data-schedule-cal-view="${id}" type="button">${label}</button>`).join("")}</div><div class="seg">${[["board","보드"],["timeline","타임라인"]].map(([id,label])=>`<button class="${layout===id?"active":""}" data-schedule-cal-layout="${id}" type="button"${calendarView==="month"&&id==="timeline"?' disabled title="주·일 보기에서 사용할 수 있어요"':""}>${label}</button>`).join("")}</div></div>`}
function scheduleList(){const byDate=new Map();state.events.forEach(event=>{const date=key(new Date(event.start));if(!byDate.has(date))byDate.set(date,[]);byDate.get(date).push(event)});if(!byDate.has(todayKey()))byDate.set(todayKey(),[]);return`<div class="uw-schedule-list">${[...byDate.keys()].sort().map(date=>`<section class="uw-date-group"><div class="uw-date-label"><span>${dayLabel(fromKey(date),true)}</span></div><div class="uw-list uw-task-main-list" data-uw-add-kind="event" data-date="${date}" data-task-drop-date="${date}">${byDate.get(date).sort((a,b)=>new Date(a.start)-new Date(b.start)).map(event=>itemMarkup("event",event,date)).join("")}${scheduleInput(date)}</div></section>`).join("")}</div>`}
...
function scheduleTimelineDay(d){const date=key(d),items=itemsForDay(date).filter(x=>x.kind==="event"),untimed=items.filter(x=>!x.timed),timed=items.filter(x=>x.timed&&x.time>=START&&x.time<END);let labels="",hits="";for(let m=START;m<END;m+=SLOT){if(m%60===0)labels+=`<span class="uw-time-label" style="top:${((m-START)/SLOT)*SLOT_H}px">${pad(m/60)}:00</span>`;hits+=`<div class="uw-time-hit" style="top:${((m-START)/SLOT)*SLOT_H}px" data-uw-add-kind="event" data-with-time="1" data-date="${date}" data-time="${m}"></div>`}const blocks=timed.map(x=>`<div class="uw-time-entry uw-item" style="top:${((x.time-START)/SLOT)*SLOT_H+1}px;height:${Math.max(18,(x.duration/SLOT)*SLOT_H-2)}px;${groupStyle(x.item)}" data-uw-kind="event" data-id="${x.item.id}" data-date="${date}" data-time="${x.time}" data-duration="${x.duration}"><button class="uw-resize-handle top" data-uw-resize="top" type="button"></button>${checkMarkup("event",x.item,date)}<span class="uw-item-title">${esc(x.item.title)}</span><button class="uw-move-handle" type="button" aria-label="길게 눌러 이동">↕</button><button class="uw-select-circle" type="button"></button><button class="uw-resize-handle bottom" data-uw-resize="bottom" type="button"></button></div>`).join("");return`<section class="uw-day${date===todayKey()?" uw-today":""}" data-date="${date}"><div class="uw-day-head"><strong>${dayLabel(d,true)}</strong></div>${scheduleAllDayPanel(date,untimed)}<div class="uw-timeline"><div class="uw-time-labels">${labels}</div><div class="uw-time-lane">${hits}${blocks}</div></div></section>`}
...
function wireScheduleViewControls(){document.addEventListener("click",e=>{const mode=e.target.closest("[data-uw-schedule-mode]"),view=e.target.closest("[data-schedule-cal-view]"),layout=e.target.closest("[data-schedule-cal-layout]"),prev=e.target.closest("#calPrev"),today=e.target.closest("#calToday"),next=e.target.closest("#calNext");if(!mode&&!view&&!layout&&!prev&&!today&&!next)return;e.preventDefault();e.stopImmediatePropagation();if(mode)schedulePageMode=mode.dataset.uwScheduleMode;if(view)calendarView=view.dataset.scheduleCalView;if(layout&&!layout.disabled)scheduleCalendarLayout=layout.dataset.scheduleCalLayout;if(prev)calendarCursor=calendarView==="month"?new Date(calendarCursor.getFullYear(),calendarCursor.getMonth()-1,1):addDays(calendarCursor,calendarView==="week"?-7:-1);if(next)calendarCursor=calendarView==="month"?new Date(calendarCursor.getFullYear(),calendarCursor.getMonth()+1,1):addDays(calendarCursor,calendarView==="week"?7:1);if(today)calendarCursor=fromKey(todayKey());renderSchedulePage()},true)}
...
function taskRowsForDate(k){return state.tasks.filter(task=>task.date===k).sort((a,b)=>+a.done-+b.done||String(a.notionStart||"").localeCompare(String(b.notionStart||""))||String(a.title).localeCompare(String(b.title),"ko"))}
function taskListMarkup(tasks,k,compact=false){return tasks.map(task=>itemMarkup("task",task,task.date||k,compact)).join("")}
...
function taskTimelineDay(d){const k=key(d),items=itemsForDay(k).filter(x=>x.kind==="task"),untimed=items.filter(x=>!x.timed),timed=items.filter(x=>x.timed&&x.time>=START&&x.time<END);let labels="",hits="";for(let m=START;m<END;m+=SLOT){if(m%60===0)labels+=`<span class="uw-time-label" style="top:${((m-START)/SLOT)*SLOT_H}px">${pad(m/60)}:00</span>`;hits+=`<div class="uw-time-hit" style="top:${((m-START)/SLOT)*SLOT_H}px" data-uw-add-kind="task" data-date="${k}" data-time="${m}"></div>`}const blocks=timed.map(x=>`<div class="uw-time-entry uw-item ${x.item.done?"done":""}" style="top:${((x.time-START)/SLOT)*SLOT_H+1}px;height:${Math.max(18,(x.duration/SLOT)*SLOT_H-2)}px;${groupStyle(x.item)}" data-uw-kind="task" data-id="${x.item.id}" data-date="${k}" data-time="${x.time}" data-duration="${x.duration}"><button class="uw-resize-handle top" data-uw-resize="top" type="button"></button>${checkMarkup("task",x.item,k)}<span class="uw-item-title">${esc(x.item.title)}</span><button class="uw-move-handle" type="button" aria-label="길게 눌러 이동">↕</button><button class="uw-select-circle" type="button"></button><button class="uw-resize-handle bottom" data-uw-resize="bottom" type="button"></button></div>`).join("");return`<section class="uw-day${k===todayKey()?" uw-today":""}" data-date="${k}"><div class="uw-day-head"><strong>${dayLabel(d,true)}</strong></div>${allDayPanel(k,untimed)}<div class="uw-timeline"><div class="uw-time-labels">${labels}</div><div class="uw-time-lane">${hits}${blocks}</div></div></section>`}
...
function renderTaskSubnav(){const nav=$("#taskPageTabs");if(!nav)return;if(taskPageMode==="list"){nav.innerHTML=`<div class="seg page-tabs">${[["all","전체"],["today","오늘"],["upcoming","예정"],["someday","언젠가"],["done","완료"]].map(([id,label])=>`<button class="${taskListTab===id?"active":""}" data-task-tab="${id}" type="button">${label}</button>`).join("")}</div>`;return}const effectiveLayout=taskCalendarView==="month"?"board":taskCalendarLayout;nav.innerHTML=`<div class="uw-task-calendar-tabs"><div class="seg">${[["month","월"],["week","주"],["day","일"]].map(([id,label])=>`<button class="${taskCalendarView===id?"active":""}" data-task-cal-view="${id}" type="button">${label}</button>`).join("")}</div><div class="seg">${[["board","보드"],["timeline","타임라인"]].map(([id,label])=>`<button class="${effectiveLayout===id?"active":""}" data-task-cal-layout="${id}" type="button"${taskCalendarView==="month"&&id==="timeline"?" disabled title=\"주·일 보기에서 사용할 수 있어요\"":""}>${label}</button>`).join("")}</div></div>`}
function renderTasksV2(){const root=$("#tasksPageList");if(!root)return;$$('[data-uw-task-mode]').forEach(button=>button.classList.toggle("active",button.dataset.uwTaskMode===taskPageMode));renderTaskSubnav();if(taskPageMode==="list"){const date=taskListTab==="today"?todayKey():taskListTab==="someday"?"":"";const rows=visibleTasks(taskListTab);root.innerHTML=`<div class="uw-list uw-task-main-list" data-uw-add-kind="task" data-date="${date}">${taskListMarkup(rows,date)}${taskListTab==="done"?"":taskListInput(date)}</div>`;return}const layout=taskCalendarView==="month"?"board":taskCalendarLayout;root.innerHTML=`<section class="uw-task-calendar-shell">${taskCalendarNav()}${taskCalendarView==="month"?taskMonthBoard():layout==="timeline"?taskTimeline():taskBoard()}</section>`}

function wireTaskViewControls(){document.addEventListener("click",e=>{const mode=e.target.closest("[data-uw-task-mode]");const tab=e.target.closest("#taskPageTabs [data-task-tab]");const view=e.target.closest("[data-task-cal-view]");const layout=e.target.closest("[data-task-cal-layout]");const prev=e.target.closest("[data-task-cal-prev]");const today=e.target.closest("[data-task-cal-today]");const next=e.target.closest("[data-task-cal-next]");if(!mode&&!tab&&!view&&!layout&&!prev&&!today&&!next)return;e.preventDefault();e.stopImmediatePropagation();if(mode)taskPageMode=mode.dataset.uwTaskMode;if(tab)taskListTab=tab.dataset.taskTab;if(view)taskCalendarView=view.dataset.taskCalView;if(layout&&!layout.disabled)taskCalendarLayout=layout.dataset.taskCalLayout;if(prev)taskCalendarCursor=taskCalendarView==="month"?new Date(taskCalendarCursor.getFullYear(),taskCalendarCursor.getMonth()-1,1):addDays(taskCalendarCursor,taskCalendarView==="week"?-7:-1);if(next)taskCalendarCursor=taskCalendarView==="month"?new Date(taskCalendarCursor.getFullYear(),taskCalendarCursor.getMonth()+1,1):addDays(taskCalendarCursor,taskCalendarView==="week"?7:1);if(today)taskCalendarCursor=fromKey(todayKey());renderTasksV2()},true)}
...
async function init(){if(document.documentElement.dataset.unifiedWorkspace)return;document.documentElement.dataset.unifiedWorkspace="1";wireDragClickGuard();wireTaskViewControls();wireScheduleViewControls();wireClicks();wireSideTabs();wireControlsV2();await renderAll()}
