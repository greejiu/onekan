...
function freezeHabitOccurrence(source,habit,date){
  if(!source||!habit||!date)return;
  const current=habitOverride(source,date,habit.id)||{},snapshot=habitOverride(source,date,habit.id,true);
  snapshot.title=current.title??habit.title;
  const start=Object.prototype.hasOwnProperty.call(current,"startMinute")?current.startMinute:habit.startMinute;
  if(start!==null&&start!==undefined&&Number.isFinite(Number(start))){snapshot.startMinute=Number(start);snapshot.duration=Number(current.duration??habit.duration)||SLOT}
  else{snapshot.startMinute=null;delete snapshot.duration}
}
function freezeCompletedHabitOccurrences(source,habit){Object.entries(source.habitDays||{}).forEach(([date,day])=>{if(day?.[habit.id]===true)freezeHabitOccurrence(source,habit,date)})}
function freezeTaskOccurrence(source,task,date){
  if(!source||!task||!date)return;
  const current=taskOverride(source,date,task.id)||{},snapshot=taskOverride(source,date,task.id,true),targetDate=Object.prototype.hasOwnProperty.call(current,"date")?current.date:date;
...
function itemsForDay(k){
  const events=schedules(k).map(event=>({kind:"event",item:event,timed:!event.allDay,time:event.allDay?null:new Date(event.start).getHours()*60+new Date(event.start).getMinutes(),duration:event.allDay?null:Math.max(SLOT,Math.round((new Date(event.end||event.start)-new Date(event.start))/60000/SLOT)*SLOT)}));
...
function timeBlockV2MinuteFromInput(value){const match=/^(\d{2}):(\d{2})$/.exec(String(value||""));if(!match)return null;const hour=+match[1],minute=+match[2];if(hour<0||hour>23||minute<0||minute>59)return null;return hour*60+minute}
...
function itemMarkup(kind,item,k,compact=false,manual=false,listDateLabel=""){
  const google=kind==="event"&&isGoogleCalendarEvent(item);
  const done=itemDoneOn(kind,item,k);
  ...
  const occurrence=(kind==="task"||kind==="event")&&item._occurrenceSource?` data-occurrence-source="${item._occurrenceSource}"`:"";
  const manualAttrs=manual&&!google?` data-manual-row data-manual-kind="${kind}" data-manual-id="${esc(item.id)}"`:"";
  const move='<button class="uw-move-handle" type="button" aria-label="길게 눌러 이동">↕</button>';
  const googleAttrs=google?` data-google-calendar-event data-google-calendar-url="${esc(item.htmlLink||"")}" title="${esc(item.calendarName||"Google 캘린더")}"`:"";
  return`<div class="uw-item uw-${kind}${done?" done":""}${compact?" compact":""}${google?" uw-google-event":""}" style="${groupStyle(item)}" data-uw-kind="${kind}" data-id="${esc(item.id)}" data-date="${k}"${occurrence}${manualAttrs}${googleAttrs} draggable="false">${checkMarkup(kind,item,k)}<span class="uw-item-title">${esc(item.title)}</span>${repeat?`<span class="uw-repeat-badge" title="${esc(repeat)}" aria-label="반복 · ${esc(repeat)}">↻</span>`:""}${time?`<span class="uw-item-time">${time}</span>`:""}${google?"":`${move}<button class="uw-select-circle" type="button" aria-label="선택"></button>`}</div>`
}
...
function openInline(host,{kind,date=null,endDate=null,time=null,duration=SLOT,editId=null,withTime=false,occurrenceSource=null,groupId=null,timeBlockId=null}={}){
  ...
  }else if(host.matches(".uw-empty-hit,.uw-time-block-v2-empty"))host.replaceWith(form);
  ...
  const commit=async next=>{
    ...
    const taskScope=old&&kind==="task"&&old.recurrence?.frequency&&!old.recurrence?.completionBased?await askTaskScope(old,sourceDate||old.date):null;
    ...
    await write(current=>{
      ...
      if(old){
        ...
        if(kind==="habit"){
          ...
          if(habitScope==="day"){
            if(value!==target.title){
              const currentOverride=habitOverride(current,editDate,target.id)||{},start=Object.prototype.hasOwnProperty.call(currentOverride,"startMinute")?currentOverride.startMinute:target.startMinute,wasDone=current.habitDays?.[editDate]?.[target.id]===true,task={id:uid(),title:value,date:editDate,done:wasDone,groupId:target.groupId||defaultGroup};
              if(start!==null&&start!==undefined&&Number.isFinite(Number(start))){const duration=Number(currentOverride.duration??target.duration)||SLOT,begin=new Date(`${editDate}T${pad(Math.floor(Number(start)/60))}:${pad(Number(start)%60)}:00`);task.notionStart=begin.toISOString();task.notionEnd=new Date(begin.getTime()+duration*60000).toISOString()}
              current.tasks.push(task);habitOverride(current,editDate,target.id,true).hidden=true;if(wasDone)current.habitDays[editDate][target.id]=false
            }
          }else{freezeCompletedHabitOccurrences(current,target);target.title=value;if(habitStartDate)target.startDate=habitStartDate;else delete target.startDate;if(habitEndDate)target.endDate=habitEndDate;else delete target.endDate;const base=habitStartDate||target.startDate||editDate;const recurrence=recurrenceFromEditor(form,base,{includeUntil:false});if(recurrence){recurrence.anchorDate=base;target.recurrence=recurrence}else delete target.recurrence}
          return
        }
        ...
        if(kind==="task"&&target.recurrence?.frequency&&taskScope==="all")freezeCompletedTaskOccurrences(current,target);
        target.title=value;
        ...
        if(recurrence){if(kind==="task")recurrence.completionBased=true;target.recurrence=recurrence}else delete target.recurrence;
        if(kind==="task"){
      ...
      if(kind==="event"){
        ...
        const siblings=current.tasks.filter(item=>(item.date||null)===(taskDate||null));
        siblings.forEach((item,index)=>{if(!Number.isFinite(Number(item.manualOrder)))item.manualOrder=(index+1)*1000});
        const siblingOrders=siblings.map(item=>Number(item.manualOrder)).filter(Number.isFinite);
...
function allDayPanel(k,items){const shown=items.slice(0,4),hidden=items.slice(4),extra=hidden.length;return`<div class="uw-all-day" data-date="${k}" data-uw-all-day-drop><span class="uw-all-day-label">하루종일</span><div class="uw-all-day-list" data-uw-add-kind="task" data-date="${k}">${shown.map(x=>itemMarkup(x.kind,x.item,k,true)).join("")||'<div class="uw-empty-hit">＋ 할일</div>'}</div>${extra?`<button class="uw-all-day-more" data-uw-all-day-more type="button" aria-expanded="false">+${extra}개 더보기</button><div class="uw-all-day-popover"><div class="uw-list">${hidden.map(x=>itemMarkup(x.kind,x.item,k,true)).join("")}</div></div>`:""}</div>`}
...
function plannerListDay(d){
  ...
  const sections=templates.map(template=>{const bucket=blockMap.get(template.id),dayOnly=Boolean(template._dateCreated);return`<section class="uw-time-block-v2-section" data-time-block-id="${esc(template.id)}"><div class="uw-time-block-v2-head"><div class="uw-time-block-v2-title"><strong>${timeBlockV2MinuteText(template.startMinute)}–${timeBlockV2MinuteText(template.endMinute)}</strong>${template.title?`<span>${esc(template.title)}</span>`:""}${dayOnly?`<span class="uw-time-block-v2-today-badge">오늘만</span>`:""}</div><button class="uw-time-block-v2-menu-button" type="button" data-uw-time-block-menu data-date="${k}" data-block-id="${esc(template.id)}" data-created="${dayOnly}" aria-label="타임블럭 메뉴">⋯</button></div><div class="uw-list uw-time-block-v2-list" data-uw-time-block-drop-list data-uw-add-kind="task" data-date="${k}" data-time-block-id="${esc(template.id)}">${timeBlockV2BlockContents(template,bucket?.timed||[],bucket?.manual||[],k,templates,assignments)}</div></section>`}).join("");
  return`<section class="uw-day uw-list-day uw-time-block-v2-day${k===todayKey()?" uw-today":""}" data-date="${k}">${head}<section class="uw-time-block-v2-section unassigned"><div class="uw-time-block-v2-head"><strong>하루종일</strong><button class="uw-time-block-day-add" type="button" data-uw-time-block-add data-date="${k}">＋ 이 날 블럭</button></div><div class="uw-list uw-time-block-v2-list" data-uw-add-kind="task" data-date="${k}" data-task-drop-date="${k}" data-uw-time-block-unassigned>${allDayMarkup}</div>${hiddenMarkup}</section>${outsideSection}${sections}</section>`
}
...
function upcomingHabitsByDate(dates){
  ...
  for(const task of state.tasks){
    if(!task.isHabit||task.done||!dates.includes(task.date))continue;
    const item={...task,_occurrenceSource:task.date,_occurrenceDate:task.date,_upcomingHabit:true};
    rowsByDate.get(task.date)?.push({kind:"task",item})
  }
...
function scheduleAllDayPanel(date,items){return`<div class="uw-all-day" data-date="${date}" data-uw-all-day-drop><span class="uw-all-day-label">하루종일</span><div class="uw-all-day-list" data-uw-add-kind="event" data-date="${date}">${items.map(x=>itemMarkup("event",x.item,date,true)).join("")||`<div class="uw-empty-hit" data-uw-add-kind="event" data-date="${date}">＋ 일정</div>`}</div></div>`}
function scheduleTimelineDay(d,index=0){
  const date=key(d),items=timelineItemsForDay(date,["event"]),untimed=items.filter(x=>!x.timed),timed=layoutTimedItems(items.filter(x=>x.timed&&x.time>=START&&x.time<END));
  let labels="",hits="";
  for(let m=START;m<END;m+=SLOT){
    if(m%60===0)labels+=`<span class="uw-time-label" style="top:${((m-START)/SLOT)*SLOT_H}px">${pad(m/60)}:00</span>`;
    hits+=`<div class="uw-time-hit" style="top:${((m-START)/SLOT)*SLOT_H}px" data-uw-add-kind="event" data-with-time="1" data-date="${date}" data-time="${m}"></div>`
  }
  const blocks=timed.map(x=>{
    if(x.kind==="session")return sessionBlockMarkup(x,date);
    const google=isGoogleCalendarEvent(x.item);
    const externalAttrs=google?` data-google-calendar-event data-google-calendar-url="${esc(x.item.htmlLink||"")}" title="${esc(x.item.calendarName||"Google 캘린더")}"`:"";
    return`<div class="uw-time-entry uw-item${google?" uw-google-event":""}" style="top:${((x.time-START)/SLOT)*SLOT_H+1}px;height:${Math.max(18,(x.duration/SLOT)*SLOT_H-2)}px;${timedColumnStyle(x)}${groupStyle(x.item)}" data-uw-kind="event" data-id="${esc(x.item.id)}" data-date="${date}" data-time="${x.time}" data-duration="${x.duration}"${externalAttrs}>${google?"":'<button class="uw-resize-handle top" data-uw-resize="top" type="button"></button>'}${checkMarkup("event",x.item,date)}<span class="uw-item-title">${esc(x.item.title)}</span>${google?"":'<button class="uw-move-handle" type="button" aria-label="길게 눌러 이동">↕</button><button class="uw-select-circle" type="button"></button><button class="uw-resize-handle bottom" data-uw-resize="bottom" type="button"></button>'}</div>`
  }).join("");
  return`<section class="uw-day${date===todayKey()?" uw-today":""}" data-date="${date}"><div class="uw-day-head"><strong>${dayLabel(d,true)}</strong></div>${scheduleAllDayPanel(date,untimed)}<div class="uw-timeline" style="height:${timelineHeight()}px"><div class="uw-time-labels">${labels}</div><div class="uw-time-lane">${hits}${currentTimeMarkup(date)}${blocks}</div></div></section>`
}
  ...
  if(name==="delete"){
    ...
    for(const record of records){
      ...
      if(record.kind==="task"){
        ...
        if(task?.recurrence?.frequency&&!task.recurrence?.completionBased){const date=record.occurrenceSource||record.date||task.date,scope=await askTaskScope(task,date,"delete");if(scope)decisions.push({...record,date,scope});continue}
      }
...
const hc=e.target.closest("[data-uw-habit-check]");if(hc){await write(s=>{const date=hc.dataset.date,id=hc.dataset.uwHabitCheck,habit=s.habitTemplates.find(item=>item.id===id);s.habitDays[date]||={};const next=!s.habitDays[date][id];if(next&&habit)freezeHabitOccurrence(s,habit,date);s.habitDays[date][id]=next});return}const check=e.target.closest("[data-uw-check]");if(check){e.stopPropagation();let onekanCheckedOn=false;await write(s=>{if(check.dataset.uwCheck==="task"){const t=s.tasks.find(x=>x.id===check.dataset.id);if(t){if(t.recurrence?.frequency){completeRepeatingTask(s,t,new Date());onekanCheckedOn=true}else if(t.done&&t.repeatRule){undoRepeatingTaskCompletion(s,t)}else{t.done=!t.done;t.completedAt=t.done?new Date().toISOString():null;onekanCheckedOn=t.done}}}else{const date=check.dataset.date,id=check.dataset.id,habit=s.habitTemplates.find(item=>item.id===id);s.habitDays[date]||={};const next=!s.habitDays[date][id];if(next&&habit)freezeHabitOccurrence(s,habit,date);s.habitDays[date][id]=next}});if(onekanCheckedOn)playCheckSound("check");return}const del=e.target.closest("[data-uw-delete-habit]");if(del){await action("delete",[{kind:"habit",id:del.dataset.uwDeleteHabit,date:todayKey()}]);return}const dup=e.target.closest("[data-uw-duplicate-habit]");if(dup){await action("duplicate",[{kind:"habit",id:dup.dataset.uwDuplicateHabit,date:todayKey()}]);return}const item=e.target.closest(".uw-item[data-uw-kind]");if(item){if(Date.now()<suppressItemClickUntil)return;if(coarse()){toggleSelection(item);return}if(!e.target.closest(".uw-item-title,.uw-habit-title"))return;openInline(item,{kind:item.dataset.uwKind,date:item.dataset.date,editId:item.dataset.id,occurrenceSource:item.dataset.occurrenceSource||null,withTime:item.dataset.uwKind==="event"&&calendarView==="day"});return}const add=e.target.closest("[data-uw-add-kind]");if(add&&!e.target.closest(".uw-item,.uw-inline-form")){const kind=add.dataset.uwAddKind,date=add.dataset.date||null,time=add.dataset.time?+add.dataset.time:null;const empty=e.target.closest(".uw-empty-hit,.uw-time-block-v2-empty"),target=empty||(add.matches(".uw-list,.uw-all-day-list,.uw-time-hit,.uw-month-cell")?add:findAddHost(kind,date,time)||add.parentElement),timeBlockId=add.dataset.timeBlockId||target.dataset.timeBlockId||null;openInline(target,{kind,date,time,groupId:add.dataset.groupId||target.dataset.groupId||null,timeBlockId,withTime:add.dataset.withTime==="1"||target.dataset.withTime==="1"})}},true);
document.addEventListener("contextmenu",e=>{const item=e.target.closest(".uw-item[data-uw-kind]");if(!item||item.hasAttribute("data-google-calendar-event"))return;e.preventDefault();const menu=$("#uwContext");const convertLabel=item.dataset.uwKind==="task"?"일정으로 바꾸기":item.dataset.uwKind==="event"?"할일로 바꾸기":null;menu.innerHTML='<button data-c="duplicate">복제</button><button data-c="group">영역</button>'+(convertLabel?`<button data-c="convert">${convertLabel}</button>`:'')+'<button class="danger" data-c="delete">삭제</button>';menu.style.left=`${Math.min(innerWidth-170,e.clientX)}px`;menu.style.top=`${Math.min(innerHeight-190,e.clientY)}px`;menu.classList.add("open");menu.onclick=async ev=>{const b=ev.target.closest("[data-c]");if(!b)return;menu.classList.remove("open");await action(b.dataset.c,[{kind:item.dataset.uwKind,id:item.dataset.id,date:item.dataset.date,occurrenceSource:item.dataset.occurrenceSource||null}])}});document.addEventListener("pointerdown",e=>{if(!e.target.closest("#uwContext"))$("#uwContext")?.classList.remove("open")})
}
...
async function dragMoveScope(kind,id,occurrenceSource,date){
  ...
  if(kind==="task"){
    ...
    if(!task)return null;
    if(!task.recurrence?.frequency||task.recurrence?.completionBased)return"all";
  ...
  return await askEventScope("change",event,sourceDate)
}
...
async function saveTimedChange(kind,id,date,startMinute,duration,occurrenceSource=date,planDate="",planToken="",forcedScope=null){
  if(kind==="habit"){
    const sourceDate=occurrenceSource||date,habit=state?.habitTemplates.find(item=>item.id===id),scope=forcedScope||(habit?await askHabitScope("change",habit,sourceDate):null);
    if(!habit||!scope)return;
    await write(current=>{clearMovedPlan(current,planDate,planToken);const target=current.habitTemplates.find(item=>item.id===id);if(!target)return;if(scope==="day"){const override=habitOverride(current,sourceDate,id,true);override.startMinute=startMinute;override.duration=duration}else{freezeCompletedHabitOccurrences(current,target);target.startMinute=startMinute;target.duration=duration}});
    return
  }
  if(kind==="task"){
    const task=state?.tasks.find(item=>item.id===id);
    if(!task)return;
    if(task.recurrence?.frequency&&!task.recurrence?.completionBased){
      const sourceDate=occurrenceSource||date||task.date,scope=forcedScope||await askTaskScope(task,sourceDate);
      if(!scope)return;
      await write(current=>{
        ...
        freezeCompletedTaskOccurrences(current,target);
  ...
  if(event.recurrence?.frequency){
    const sourceDate=occurrenceSource||date||key(new Date(event.start)),scope=forcedScope||await askEventScope("change",event,sourceDate);
    if(!scope)return;
...
async function saveUntimedChange(kind,id,date,occurrenceSource=date,planDate="",planToken="",forcedScope=null){
  ...
  if(kind==="task"){
    ...
    if(!task)return;
    if(task.recurrence?.frequency&&!task.recurrence?.completionBased){
      const sourceDate=occurrenceSource||task.date;
      const scope=forcedScope||await askTaskScope(task,sourceDate);
      if(!scope)return;
      await write(current=>{
        clearMovedPlan(current,planDate,planToken);
        const target=current.tasks.find(item=>item.id===id);
...
async function saveDateOnlyChange(kind,id,date,occurrenceSource=date,planDate="",planToken="",forcedScope=null){
  ...
  if(kind==="task"){
    ...
    if(task.recurrence?.frequency&&!task.recurrence?.completionBased&&!scope)scope=await askTaskScope(task,sourceDate);
    if(task.recurrence?.frequency&&!task.recurrence?.completionBased&&scope==="all"&&!date){showToast("반복 전체는 언젠가로 옮길 수 없어요. ‘이 할일만’을 선택해 주세요.");return}
  }else if(kind==="event"){
    const event=state?.events.find(item=>item.id===id);
    if(!event)return;
    sourceDate=occurrenceSource||key(new Date(event.start));
    if(event.recurrence?.frequency&&!scope)scope=await askEventScope("change",event,sourceDate)
  }
  if(!scope)scope="all";
  await write(s=>{
    ...
    if(kind==="task"){
      ...
      if(task.recurrence?.frequency)freezeCompletedTaskOccurrences(s,task);
      task.date=date;
...
async function init(){
  ...
  document.addEventListener("onekan:google-calendar-changed",()=>scheduleRender(20));
  initGoogleCalendar().catch(error=>console.warn("Google 캘린더 초기화 실패",error));
