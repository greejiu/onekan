...
function itemMarkup(kind,item,k,compact=false,manual=false,listDateLabel=""){
  ...
  return`<div class="uw-item uw-${kind}${done?" done":""}${compact?" compact":""}" style="${groupStyle(item)}" data-uw-kind="${kind}" data-id="${item.id}" data-date="${k}"${occurrence}${manualAttrs} draggable="false">${checkMarkup(kind,item,k)}<span class="uw-item-title">${esc(item.title)}</span>${repeat?`<span class="uw-repeat-badge" title="${esc(repeat)}" aria-label="반복 · ${esc(repeat)}">↻</span>`:""}${time?`<span class="uw-item-time">${time}</span>`:""}${move}<button class="uw-select-circle" type="button" aria-label="선택"></button></div>`
}
...
function renderHome(){renderPlanner();renderSomeday();renderUpcoming();renderSideTab();renderOverdue();wireHomeColumnHeightSync();syncHomeColumnHeight()}
