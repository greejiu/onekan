...
function upcomingGroups(limit=7,horizon=366){const groups=[];let count=0;for(let i=1;i<=horizon&&count<limit;i++){const date=key(addDays(fromKey(todayKey()),i)),rows=[...schedules(date).map(item=>({kind:"event",item})),...taskOccurrencesForDate(date).filter(task=>!taskDoneOn(task,date)).map(item=>({kind:"task",item}))];if(!rows.length)continue;const visible=rows.slice(0,limit-count);groups.push({date,rows:visible});count+=visible.length}return groups}
function renderUpcoming(){const root=$("#upcomingList");if(!root)return;const groups=upcomingGroups();root.innerHTML=groups.length?groups.map(({date,rows})=>`<div class="uw-date-group"><div class="uw-date-label"><span>${dayLabel(fromKey(date),true)}</span></div><div class="uw-list" data-uw-add-kind="task" data-date="${date}">${rows.map(x=>itemMarkup(x.kind,x.item,date)).join("")}</div></div>`).join(""):'<div class="empty">다가오는 일정이 없어요.</div>'}
