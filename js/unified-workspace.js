...
function openInline(host,{kind,date=null,endDate=null,time=null,duration=SLOT,editId=null,withTime=false,occurrenceSource=null,groupId=null}={}){
  ...
  const commit=async next=>{
    ...
    const taskScope=old&&kind==="task"&&old.recurrence?.frequency?await askTaskScope(old,sourceDate||old.date):null;
...
async function saveDateOnlyChange(kind,id,date,occurrenceSource=date,planDate="",planToken="",forcedScope=null){
  ...
  if(kind==="task"){
    ...
    if(task.recurrence?.frequency&&!scope)scope=await askTaskScope(task,sourceDate);
