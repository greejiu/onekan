...
const hc=e.target.closest("[data-uw-habit-check]");if(hc){await write(s=>{const date=hc.dataset.date,id=hc.dataset.uwHabitCheck,habit=s.habitTemplates.find(item=>item.id===id);s.habitDays[date]||={};const next=!s.habitDays[date][id];if(next&&habit)freezeHabitOccurrence(s,habit,date);s.habitDays[date][id]=next});return}const check=e.target.closest("[data-uw-check]");if(check){e.stopPropagation();await write(s=>{if(check.dataset.uwCheck==="task"){const t=s.tasks.find(x=>x.id===check.dataset.id);if(t){if(t.recurrence?.frequency){completeRepeatingTask(s,t,new Date())}else if(t.done&&t.repeatRule){undoRepeatingTaskCompletion(s,t)}else{t.done=!t.done;t.completedAt=t.done?new Date().toISOString():null}}}else{const date=check.dataset.date,id=check.dataset.id,habit=s.habitTemplates.find(item=>item.id===id);s.habitDays[date]||={};const next=!s.habitDays[date][id];if(next&&habit)freezeHabitOccurrence(s,habit,date);s.habitDays[date][id]=next}});return}const del=e.target.closest("[data-uw-delete-habit]");if(del){await action("delete",[{kind:"habit",id:del.dataset.uwDeleteHabit,date:todayKey()}]);return}const dup=e.target.closest("[data-uw-duplicate-habit]");if(dup){await action("duplicate",[{kind:"habit",id:dup.dataset.uwDuplicateHabit,date:todayKey()}]);return}const item=e.target.closest(".uw-item[data-uw-kind]");if(item){if(Date.now()<suppressItemClickUntil)return;if(coarse()){toggleSelection(item);return}if(!e.target.closest(".uw-item-title,.uw-habit-title"))return;openInline(item,{kind:item.dataset.uwKind,date:item.dataset.date,editId:item.dataset.id,occurrenceSource:item.dataset.occurrenceSource||null,withTime:item.dataset.uwKind==="event"&&calendarView==="day"});return}const add=e.target.closest("[data-uw-add-kind]");if(add&&!e.target.closest(".uw-item,.uw-inline-form")){const kind=add.dataset.uwAddKind,date=add.dataset.date||null,time=add.dataset.time?+add.dataset.time:null;const empty=e.target.closest(".uw-empty-hit,.uw-time-block-v2-empty"),target=empty||(add.matches(".uw-list,.uw-all-day-list,.uw-time-hit,.uw-month-cell")?add:findAddHost(kind,date,time)||add.parentElement),timeBlockId=add.dataset.timeBlockId||target.dataset.timeBlockId||null;openInline(target,{kind,date,time,groupId:add.dataset.groupId||target.dataset.groupId||null,timeBlockId,withTime:add.dataset.withTime==="1"||target.dataset.withTime==="1"})}},true);
