...
assert.ok(u.includes('data-id="${x.item.id}" data-date="${k}" data-time-block-anchor="${esc(timeBlockV2EntryToken(x,k))}"'),'exact timeline cards should expose anchor token');
assert.ok(!u.includes('data-date="${k}" data-time-block-anchor="${esc(timeBlockV2EntryToken(x,k))}" data-time="${m}"'),'timeline hit cells must never reference exact-row x');
...
assert.match(i,/unified-workspace\.js\?v=47/,'JS cache should be bumped after anchor fix');
assert.match(i,/unified-workspace\.css\?v=39/,'CSS cache should remain current');
